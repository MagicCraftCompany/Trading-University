...
const CustomCheckoutForm: React.FC<CustomCheckoutFormProps> = ({
  ...
  // Countries list for dropdown - sorted alphabetically
  const countries = [
  ...
  const registerUser = async () => {
    try {
      ...
      // Don't store token and user info during registration
      // Just return the data for payment processing
      if (!data.token) {
    ...
    try {
      ...
      try {
        ...
        if (paymentData.url) {
          ...
          window.location.href = `/payment-confirmation?plan_name=${encodeURIComponent(productName)}`;
        }
      } catch (paymentError) {
          ...
          <div className="relative">
            ...
            >
              {countries.map((country) => (
                <option key={country.code} value={country.code} className="color-force-white">
                  {country.name}
                </option>
              ))}
            </select>
